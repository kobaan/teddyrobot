...
wheelWidth     = 18;     // The width (or thickness) of the wheel at the rim.
tireCSDiameter = 5;     // Cross-sectional diameter (CS) -- How thick is the tire rubber?
tireID         = 160;    // Internal diameter (ID) -- How wide is the inside opening? 
...
hubHeight            = 21;    // The total height of the hub
...
baseFilletRadius     = 2.5;     // The radius of the fillet (rounded part) between the hub and wheel. 
topFilletRadius      = 3;     // The radius of the fillet (rounded part) at the top of the hub. 
...
shaftFlatDiameter    = 4;          // The diameter of the motor shaft at the flat, or shaftDiameter for no flat.
...
$fn = 60;  // Default quality for most circle parts. 
...
m3tap=3;
m3tap_mount_offset=25/2;
m3tap_mount_angle=60;
m3_height=5;
...
// bore
color("orange")
translate([boretap_offset,0,bore_heigth/2])
cylinder(r=boretap/2,h=bore_heigth,center=true,$fn=quality);

// taps
color("red")
translate([0,m3tap_mount_offset,m3_height/2])
cylinder(r=m3tap/2,h=m3_height,center=true,$fn=quality);

color("blue")
rotate([0,0,180+m3tap_mount_angle]) 
translate([0,m3tap_mount_offset,m3_height/2])
cylinder(r=m3tap/2,h=m3_height,center=true,$fn=quality);

color("green")
rotate([0,0,-m3tap_mount_angle]) 
translate([0,-m3tap_mount_offset,m3_height/2])
cylinder(r=m3tap/2,h=m3_height,center=true,$fn=quality);
}
...
bearing(pos=[0,0,-10], angle=[0,0,0], model=608, outline=false, material=Brass, sideMaterial=Steel);

{
color("red") translate([-20,-20,23.5]) cube([40,90,8]);
union() {
// 8mm Rad Achse
color("silver") cylinder(h=60,r=8/2);
// 5mm Motor Achse
color("silver") translate([0,60,13]) cylinder(h=10,r=5/2);
// 6mm Motor Achse
color("silver") translate([0,60,23-2.5]) cylinder(h=2.5,r=6/2);
// 30mm Motor Mantel
color("silver") translate([0,60-5.5,23]) cylinder(h=23,r=30/2);
//emg30_taps();
}
}
// grosses Zahnrad
color("lightblue") translate([0,0,3]) import("T5-28-v1.stl");
// kleines Zahnrad
#color("lightblue") translate([0,60,22.5]) rotate([0,180,0]) import("T5-16-v1.stl");
